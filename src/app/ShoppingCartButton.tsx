                    ...
                    <div className="flex grow  flex-col space-y-5 overflow-y-auto pt-1">
                        <ul className="space-y-5">
                            {cartQuery.data?.lineItems?.map((item) => (
                                <ShoppingCartItem key={item._id} item={item} onProductLinkClicked={() => setSheetOpen(false)} />
                            ))}
                        </ul>
                        {cartQuery.isPending && (
                            ...
                            !cartQuery.isPending && !cartQuery.data?.lineItems?.length && (
                                <div className="flex grow items-center justify-center text-center">
                                    <div className="space-y-1.5">
                                        ...
                                        <Link href={"/shop"} className="text-primary hover:underline" onClick={() => setSheetOpen(false)}>
                                            Começe a comprar agora
                                        </Link>
                                    </div>
                                </div>
                            )
    ...
    return <li className="flex items-center gap-3">
        <div className="relative size-fit flex-none">
            <Link href={`/products/${slug}`} onClick={onProductLinkClicked}>
                <WixImage mediaIdentifier={item.image} width={110} height={110} alt={item.productName?.translated || "Imagem do produto"} className="flex-none bg-secondary" />
            </Link>
        <button
        ...
        <div className="space-y-1.5 text-sm">
            ...
            {!!item.descriptionLines?.length && (
                <p>{item.descriptionLines.map((line) => line.colorInfo?.translated || line.plainText?.translated).join(", ")}</p>
            )}
            ...
            <div className="flex items-center gap-1.5">
                ...
                {quantityLimitReached && <span>Quantidate limite excedida</span>}
            </div>
        </div>
    </li>
}